...
export async function createUser(currentState, formData) {
  ...
  if (error) {
    ...
    console.log(error);
    return { error: true, message: 'Recieved invalid credentials. Please try again.' };
  }

  const { id, username, role, name, email, password } = validatedData;

  // make sure that username and email aren't taken already
  const userExists = await getUserByEmail(email) || await getUserByUsername(username);
  if (userExists) {
    return { message: 'User already exists. Try a different username or email.' }
  }
  ...
  try {
    const { rows } = await pool.query(insertQuery, [id, username, role, name, email, hashedPassword]);
  }
  catch (e) {
...
// R: Read
export const getUserByUsername = React.cache(
  async function (username) {
    try {
      const { rows: userData } = await pool.query('select * from users where username=$1', [username]);
      return userData[0];
    }
    catch (e) {
      console.log(e);
...
const getUserByEmail = React.cache(
  async function (email) {
    try {
      const { rows: userData } = await pool.query('SELECT * FROM users WHERE email=$1 LIMIT 1;', [email]);
      return userData[0];
    }
    catch (e) {
      console.log(e);
...
export async function setUserRole(id, role) {
  ...
  try {
    const { rows } = await pool.query(updateRoleQuery, [role, id, otherRoleValue]);
  }
  catch (e) {
    console.log(e);
    return { error: true };
...
export async function updateUserData(prevState, formData) {
  ...
  const isReqValid = await validateRequestAuths(formData.get('username'), 'updateUserData');
...
export async function changePassword(prevState, formData) {
  // check authenticity and authority of request
  const isReqValid = await validateRequestAuths(formData.get('username'), 'changePassword');
  if (isReqValid?.serverError) {
    return isReqValid;
  }
  const { user } = isReqValid;
  ...
  try {
    const { rows } = await client.query('select password from users where id=$1', [user.id]);
...
export async function deleteUser(prevState, formData) {
  // check authenticity and authority of request
  const isReqValid = await validateRequestAuths(formData.get('username'), 'deleteUser');
  if (isReqValid?.serverError) {
    return isReqValid;
  }
  const { user } = isReqValid;
  ...
  if (error) {
    ...
    console.log({ serverError }, 'validation error');
    return { serverError, message: 'Failed to delete account. Please try again.' };
  }
